...
export default function LeadsListPage() {
  ...
  useEffect(() => {
    // The pipeline view fetches its own data (LeadPipelineBoard) — no need
    // for this page's own list request while it's showing.
    if (view !== 'list') return
    loadLeads()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view, page, statusFilter, committedSearch])
  ...
  const handleViewChange = (nextView: 'list' | 'pipeline') => {
    const next = new URLSearchParams(searchParams)
    if (nextView === 'pipeline') next.set('view', 'pipeline')
        ...
        <div className="flex items-center gap-3">
          <div className="inline-flex rounded-md border border-gray-300 bg-white p-0.5">
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded px-2.5 py-1.5 text-sm font-medium ${
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded px-2.5 py-1.5 text-sm font-medium ${
                view === 'pipeline' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:text-gray-900'
              }`}
            >
